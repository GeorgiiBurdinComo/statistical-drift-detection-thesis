...
\abstract{%
Closed LLM APIs are used in this thesis to classify social-media posts as relevant or not relevant to a marketing campaign. Because provider-side updates may alter model behaviour over time, the same task is evaluated repeatedly on a fixed labelled dataset. A reduced monitoring subset is used to lower evaluation cost, and changes are tested with the exact McNemar test on paired correctness outcomes. Candidate models are additionally compared with a cost-aware decision rule.
...
\newcommand{\CodeRepoBase}{https://github.com/GeorgiiBurdinComo/llm-batch-eval/blob/main/}
\newcommand{\ThesisRepoBase}{https://github.com/GeorgiiBurdinComo/statistical-drift-detection-thesis/blob/main/}
\newcommand{\codepath}[1]{\href{\CodeRepoBase#1}{\nolinkurl{#1}}}
\newcommand{\thesispath}[1]{\href{\ThesisRepoBase#1}{\nolinkurl{#1}}}
\newcommand{\reproanchor}[1]{%
  \par\smallskip\noindent{\footnotesize\textit{Reproducibility anchor.} #1\par}%
}
