...
The corresponding dashboard controls are shown earlier in Figure~\ref{fig:utility-overview}. Figures~\ref{fig:utility-table-lambda10}--\ref{fig:utility-vs-lambda-highlight} then illustrate the concrete per-model utilities and the behaviour of $U_t(m)$ as $\lambda$ varies.
...
Figure~\ref{fig:overall-system-architecture} gives the high-level design. The key architectural decision is that the system separates one-time dataset construction from repeated monitoring. A manual full-benchmark run on \(N=1200\) labelled examples is used once to define the disagreement-based subset, whereas the recurring weekly loop operates only on the fixed \(n=300\) monitoring set. This keeps repeated evaluation inexpensive while preserving paired comparability across time.
...
\begin{itemize}
    \item \textbf{Reference benchmark} (\(N=1200\)): \href{https://cloud.langfuse.com/project/cmkqnvv3z016aad08kqtjim2n/datasets/cmlfi3uo0003yad07e75cd7e3}{Langfuse link};
...
\begin{figure}[H]
    ...
    \node[figboxsmall] (reference) {Reference\\Benchmark\\Langfuse\\\(N=1200\)};
...
Among these, the paired-drift and utility views are the operationally decisive ones because they turn raw traces into concrete actions: whether to keep the current setup, switch models, or attempt prompt recovery. Figure~\ref{fig:mcnemar-dashboard} shows the paired-drift interface, and Figure~\ref{fig:utility-overview} shows the utility interface used for cost-aware comparison.
...
\begin{itemize}
    \item the full reference benchmark with \(N=1200\) labelled examples;
...
\begin{itemize}
    ...
    \item deployment utility \(U_t(m)\) for corrective-action comparison.
\end{itemize}
...
\begin{enumerate}
    ...
    \item if drift is detected, compare corrective actions under \(U_t(m)\);
    \item either switch model or run GEPA;
    \item if GEPA is used, re-evaluate the recovered prompt on the same 300 examples.
\end{enumerate}

The default deployment setting is \(\lambda=10\). GEPA runs under a fixed evaluation budget, and any recovery attempt is reported through \(c_{\mathrm{new}}\), \(b_{\mathrm{new}}\), \(\Delta\)accuracy, and \(\Delta\)utility. Figure~\ref{fig:run-decision-logic} summarizes this experimental logic.
\FloatBarrier
\begin{figure}[H]
    \centering
    \begin{adjustbox}{max width=0.92\textwidth}
...
\section{Phase 3: Cost-Aware Model Selection}
...
\begin{itemize}
    \item at \(\lambda=0\), the ranking reduces to pure predictive performance;
    \item as \(\lambda\) increases, token-efficient models gain utility even if they are slightly less accurate;
    \item at the default \(\lambda=10\), the preferred action reflects an explicit compromise between performance and operating cost.
\end{itemize}

This matters because a drift response is not a binary technical fix. A more accurate fallback model may still be a poor deployment choice if the cost increase is disproportionate. Conversely, a cheaper model is only attractive if the drop in monitored performance is sufficiently small. Figure~\ref{fig:utility-overview} shows the dashboard view used to inspect these trade-offs across date windows and model sets.
...
Recovery is then measured with the same quantities used in the rest of the chapter: \(c_{\mathrm{new}}\), \(b_{\mathrm{new}}\), \(\Delta\)accuracy, and \(\Delta\)utility. This is what makes prompt recovery commensurable with model switching rather than a separate qualitative intervention.
...
\section{Cost-Sensitive Utility Comparison}
\label{sec:lambda-utility-comparison}
...
\]
where \(\lambda\) is the current cost-sensitivity parameter. The chapter reports \(\Delta U = U_{t_1}(m^\ast) - U_{t_0}(m^\ast)\) alongside \(\Delta \text{acc}\) so that degradation is visible both in accuracy and in utility space.
...
\begin{itemize}
    ...
    \item the change in deployment utility \(\Delta U_{\mathrm{prompt}} = U(P_{\mathrm{new}}) - U(P_{\mathrm{old}})\) at the chosen \(\lambda\);
