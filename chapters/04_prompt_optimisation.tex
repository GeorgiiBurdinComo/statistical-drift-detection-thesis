...
\begin{tikzpicture}[node distance=5mm, font=\small]
  ...
  % Feedback: horizontal bar below both branches, then right-and-up to budget
  \draw (disc.south) -- ++(0,-6mm);
  \draw (add.south)  -- ++(0,-6mm);
  \draw ($(disc.south)+(0,-6mm)$) -- ($(add.south)+(0,-6mm)$);
  \draw[figarrow] ($(add.south)+(0,-6mm)$) -- ++(8mm,0) |- (budget.east);
\end{tikzpicture}
...
\begin{tikzpicture}[
  ...
  \node[apobox, right=7mm of rev]  (cmp)  {Compare\\with fallbacks};
