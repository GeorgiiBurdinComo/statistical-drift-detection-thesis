...
The counts and charges are observed on \(\mathcal{B}_t\), while the penalties are scenario inputs. Thus \(L_t(m)\) is the expected cost of the declared penalty scenario on the common complete cases. Changing penalties changes the decision problem.
...
\end{align}
Increasing \(\lambda\) raises error importance relative to API cost; changing \(w\) redistributes that penalty between error types. Figure~\ref{fig:cost-penalty-map} shows the same decision geometry in the raw penalty coordinates \(C_{\mathrm{FN}}\) and \(C_{\mathrm{FP}}\). Panel~(a) uses linear axes from \(0\) to \(10\) USD; panel~(b) uses logarithmic axes over \(10^{-2}\) to \(10\) USD so that the near-origin transitions remain visible.
...
The breakpoint is usable only with nonzero denominator and \(\lambda^\star>0\); evaluate the loss sign on either side.
...
\end{equation}
Require a nonzero denominator and \(w^\star\ge0\). Across pairs, these boundaries divide the \((w,\lambda)\) plane into regions with a constant minimiser.
...
The loss map provides regions of preferred models under different penalty settings. A high-volume collection task can legitimately choose a low-cost candidate when the absolute penalty of a missed or extra post is small. A default operating point can select a stable cost-quality compromise. A report with no manual review should first impose a precision constraint and then compare recall and cost. These are policy choices; the measured candidate counts decide which model is feasible in each scenario.
...
An empty feasible set returns no candidate; tied optima require a declared secondary rule; an undefined required rate cannot establish feasibility.

\section{Decision scope and monitoring handoff}

The ranking is conditional on the candidate roster, common full-benchmark cases, observed charges, and declared preferences; it has no production-sampling interpretation. The sentinel panel supplies an alert, never the FP/FN counts for candidate ranking. Close losses require sensitivity and operational review.
