...
\noindent Monitoring flags a candidate for review. This chapter ranks replacements on common full-benchmark cases. The result is conditional on explicit error prices and observed inference charges. It applies cost-sensitive decision analysis after inference rather than training a cost-sensitive classifier~\cite{elkan2001cost,turney2000cost}.
...
The counts and charges are observed on \(\mathcal{B}_t\), while the penalties are scenario inputs. Thus \(L_t(m)\) is assigned benchmark loss, not expected production cost; changing penalties changes the decision problem.
...
Against \texttt{gpt-5-nano}, \texttt{gpt-5-mini} has \((\Delta c,\Delta\mathrm{FP},\Delta\mathrm{FN})=(0.0009483,1,-42)\) on \(n_t=1113\). Mini costs more per call and makes one additional false positive, but avoids 42 false negatives. At \(w=0.2\), the break-even severity is \(\lambda^\star=0.1712\) USD; the illustrative point \((w,\lambda)=(0.2,0.186)\) narrowly favours mini.
