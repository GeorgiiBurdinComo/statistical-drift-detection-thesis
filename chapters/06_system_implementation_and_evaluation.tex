...
The inference task is identical across providers: classify each example as campaign-relevant (binary \texttt{campaign\_relevant}) using the same multimodal input and the same JSON-schema-constrained output shape. What differs is only the adapter layer required by each provider API. Table~\ref{tab:provider-adapters} therefore compares transport-level mapping decisions, not task semantics or evaluation criteria.

\begin{table}[htbp]
\centering
\footnotesize
\caption{Provider-specific adapter mapping for one shared classification task (\texttt{campaign\_relevant}) and one shared request body (multimodal \texttt{input} + JSON schema).}
